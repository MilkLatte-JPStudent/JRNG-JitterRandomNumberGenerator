...
if __name__ == "__main__":
    while True:
        total_count = 1 << 1
        value = [struct.pack('<2048h', *generate_32_chaos(count=64)) for _2 in range(total_count)] + [struct.pack('<2048h', *generate_32_chaos(count=64)) for _2 in range(total_count)] + [struct.pack('<2048h', *generate_32_chaos(count=64)) for _2 in range(total_count)]
        binary_data = b''.join(value)
        sys.stdout.buffer.write(binary_data)
        sys.stdout.buffer.flush()
    """for _2 in range(100):
        print(int(generate_chaos_number(min_val=-32768, max_val=32767, layers=75)))
        print(str(int(time.time_ns() % 10000000 / 1000)))
        print(str(get_physical_jitter_seed(_2+ik)))"""